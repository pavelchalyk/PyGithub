...
class Requester:
    ...
    def __log(self, verb, url, requestHeaders, input, status, responseHeaders, output):
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.debug("%s %s://%s%s %s %s ==> %i %s %s", str(verb), self.__scheme, self.__hostname, str(url), str(requestHeaders), str(input), status, str(responseHeaders), str(output.encode("UTF-8")))
